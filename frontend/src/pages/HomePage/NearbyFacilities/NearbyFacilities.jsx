...
// Component BookingModalInline đã được sửa để giống với SymptomResultsPage
const BookingModalInline = ({ 
  facility, 
  isOpen, 
  onClose, 
  onConfirm 
            ...
            <div className="booking-facility-info">
              ...
              <div className="facility-details">
                <span className="badge bg-light text-dark me-2">
                  ...
                  {facility?.distance ? `${facility.distance} km` : 'Chưa xác định'}
                </span>
                <span className="badge bg-light text-dark me-2">
              ...
              <div className="booking-step-content">
                ...
                <div className="booking-summary">
                  ...
                  <div className="summary-section">
                    ...
                    <p><strong>Cơ sở:</strong> {facility?.name}</p>
                ...
                <div className="booking-confirmation">
                  ...
                  <div className="confirmation-details">
                    <p><i className="bi bi-building me-2"></i>
                      <strong>Cơ sở:</strong> {facility?.name}
                    </p>
            ...
            {bookingStep < 3 && !bookingConfirmed ? (
              <>
                <button
                  ...
                  onClick={bookingStep === 1 ? onClose : handlePrevStep}
                >
...
const NearbyFacilities = ({
  ...
  // State cho booking modal - ĐÃ CẬP NHẬT
  ...
  // Dữ liệu mẫu đánh giá theo bệnh viện (giữ nguyên)
  ...
  // Dữ liệu mẫu cho demo (giữ nguyên)
  ...
  // Sử dụng facilities từ props hoặc mẫu
  ...
  // 3. Lấy vị trí người dùng (simplified)
  const fetchUserLocation = async () => {
    setIsLoadingLocation(true);
    setLocationError('');

    setTimeout(() => {
      // Luôn sử dụng vị trí mặc định cho demo
  ...
  // 4. Cập nhật khoảng cách cho các cơ sở
  ...
  const handleManualLocationInput = async () => {
    ...
    setTimeout(() => {
      ...
      // Hiển thị danh sách bệnh viện sau khi nhập địa chỉ
  ...
  // 7. Hiển thị thông tin nguồn vị trí
  const getLocationSourceText = () => {
    switch (locationSource) {
      case 'manual':
        return 'Địa chỉ thủ công';
      case 'default':
        return 'Vị trí mặc định';
      default:
  ...
  // 9. Xử lý click xem đánh giá
  const handleViewReviews = (facilityId) => {
    if (expandedFacilityId === facilityId) {
      // Nếu đang mở thì đóng lại
      setExpandedFacilityId(null);
    } else {
      // Mở đánh giá của bệnh viện này
      setExpandedFacilityId(facilityId);
    }

    // Reset filter khi mở đánh giá mới
    setReviewFilter('all');
  };

  // 10. Lấy đánh giá theo facilityId và filter
  ...
  // 12. Tính toán distribution rating
  ...
  // 15. HÀM MỚI: Đóng modal đặt lịch
  ...
  // 16. HÀM MỚI: Xử lý thay đổi form
  const handleBookingFormChange = (e) => {
    const { name, value } = e.target;
    setBookingForm(prev => ({
      ...prev,
  ...
  // 17. HÀM MỚI: Chuyển bước đặt lịch
  const handleNextStep = () => {
    if (bookingStep === 1) {
      // Validate step 1
      if (!bookingForm.name || !bookingForm.phone || !bookingForm.date) {
  ...
  return (
    <div className="nearby-facilities-section card">
      <div className="card-body">
              ...
              {isLoadingLocation ? (
                ...
                <div className="location-details">
                  <span className="location-source">
                    <i className={`bi ${locationSource === 'manual' ? 'bi-pencil-square' : 'bi-geo'
                      }`}></i>
        ...
        {/* Bộ lọc sắp xếp (chỉ hiện khi có dữ liệu) */}
        {showFacilities && (
          <div className="sorting-controls">
            <div className="distance-info">
              <span className="accuracy-info">
                <i className="bi bi-info-circle"></i>
                Hiển thị {sortedFacilities.length} cơ sở trong khu vực của bạn
              </span>
            </div>
          </div>
        )}
